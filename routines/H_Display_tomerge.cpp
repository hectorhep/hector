...
void display_lhcbeams(float length=500., string beam1file="data/LHCB1IR5_v6.500.tfs", string beam2file="data/LHCB2IR5_v6.500.tfs", char * ipname = "IP5") {
	...
	beam_profile_x->GetXaxis()->SetTitleSize(0.09);
	beam_profile_x->GetYaxis()->SetTitleSize(0.09);
	beam_profile_x->GetXaxis()->SetLabelSize(0.08);
	beam_profile_x->GetYaxis()->SetLabelSize(0.08);
	...
	beam_profile_y->GetXaxis()->SetTitle("s [m]");
        beam_profile_y->GetXaxis()->SetTitleSize(0.09);
        beam_profile_y->GetYaxis()->SetTitleSize(0.09);
        beam_profile_y->GetXaxis()->SetLabelSize(0.08);
        beam_profile_y->GetYaxis()->SetLabelSize(0.08);
...
/// \brief Shows both LHC beams aside, from the top and from the side
void display_lhcbeam_eloss(float length=500., string beam1file="data/LHCB1IR5_v6.500.tfs", char * ipname = "IP5") {
...
//	setTDRStyle();
	...
	TMultiGraph * beam_profile_x = new TMultiGraph("beam_profile_x","");
	TMultiGraph * beam_profile_y = new TMultiGraph("beam_profile_y","");
	TLegend * leg = new TLegend(0.15,0.35,0.4,0.5);
	...
	for(int i=0;i<50;i++) {
		...
		TGraph * ppath_x = p1.getPath(0,(i==49)?kRed:kBlack);
		TGraph * ppath_y = p1.getPath(1,color+j-1);
		if (i==0) leg->AddEntry(ppath_x," beam protons","l");
		else if (i==49) { 
			ppath_x->SetLineWidth(2);
			leg->AddEntry(ppath_x," E_{loss}=200 GeV","l");
			}
   	...
   	TCanvas* ccc = new TCanvas("ccc","the particle test canvas",10,10,710,290);
//   	TCanvas* ccc = new TCanvas("ccc","the particle test canvas",10,10,710,710);
	ccc->cd();
//	ccc->Divide(1,2);
//	ccc->cd(1); 
	gPad->SetGrid();
	...
	beam_profile_x->GetYaxis()->SetTitleOffset(1.1);

	beam_profile_x->GetXaxis()->SetTitleSize(0.06);
	beam_profile_x->GetYaxis()->SetTitleSize(0.06);
	beam_profile_x->GetXaxis()->SetLabelSize(0.06);
	beam_profile_x->GetYaxis()->SetLabelSize(0.06);
...
//	beamline1->drawX(beam_profile_x->GetYaxis()->GetXmin()/5.,beam_profile_x->GetYaxis()->GetXmax()/5.,1E-3);
	...
	TPaveText * pave = new TPaveText(0.15,0.15,0.3,0.3,"NDC");
        pave->SetFillColor(0);
        pave->SetFillStyle(0);
        pave->SetBorderSize(0);
        pave->AddText("Hector");
        pave->SetTextAlign(11);
        pave->SetTextSizePixels(25);
        pave->Draw();
...
void display_loss100GeV() {
        ...
        beamline->fill("data/LHCB1IR5_v6.500.tfs",1,"IP5");
        beamline->offsetElements(120,-0.097);
        beamline->showElements();
        beamline->calcMatrix();

	TMultiGraph * beam_profile_x = new TMultiGraph("beam_profile_x","");
	TLegend *leg = new TLegend(0.657,0.576,0.88,0.95,"proton E_{loss}");
        leg->SetBorderSize(0);

        for(int i=0;i<3;i++) {
                H_BeamParticle p1;
		float energyloss = 0.;
		if (i==1) energyloss= 50.;
		else if (i==2) energyloss = 500.;
		p1.emitGamma(energyloss,0.);
                p1.setPosition(-500.,0,-CRANG,0,0);
                p1.computePath(beamline,1);

                TGraph * ppath_x = p1.getPath(0,(i==1)?kRed:(i==2)?kBlue:kBlack);
		ppath_x->SetLineWidth((i>0)?3:1);
		ppath_x->SetLineStyle((i==1)?7:1);
                beam_profile_x->Add(rescale_tgraph(ppath_x,1E-3));
		char text[500];
		sprintf(text,"%d GeV",(int)energyloss);
		leg->AddEntry(ppath_x,text,"l");
	}
	...
	ccc->cd();
        ...
        beam_profile_x->GetXaxis()->SetTitle("s [m]");
        beam_profile_x->GetYaxis()->SetTitle("x [mm]");
        beam_profile_x->GetYaxis()->SetTitleOffset(0.44);
        beam_profile_x->GetYaxis()->SetTitleSize(0.09);
        beam_profile_x->GetYaxis()->SetLabelSize(0.08);
        beam_profile_x->GetXaxis()->SetTitleSize(0.09);
        beam_profile_x->GetXaxis()->SetLabelSize(0.08);
        beam_profile_x->GetXaxis()->SetTitleOffset(0.84);
	beam_profile_x->GetXaxis()->SetRangeUser(205,450);
	beam_profile_x->GetYaxis()->SetRangeUser(-20,20);
	leg->Draw();
	leg->SetFillColor(0);

	TLine * rp220 = new TLine(220,2,220,20);
	rp220->SetLineWidth(4);
	rp220->Draw();

	TLine * fp420 = new TLine(420,-4,420,-20);
	fp420->SetLineWidth(4);
	fp420->Draw();
	ccc->Update();
...
//        beamline->drawX(200,450,beam_profile_x->GetYaxis()->GetXmin()/1.,beam_profile_x->GetYaxis()->GetXmax()/1.,1E-3);
//        beam_profile_x->Draw("L+");
...
void display_loss_scan() {
	...
	TMultiGraph * beam_profile_x = new TMultiGraph("beam_profile_x","");
	TLegend *leg = new TLegend(0.70,0.70,0.88,0.88,"proton E_{loss}");

        for(int i=0;i<10;i++) {
                H_BeamParticle p1;
		float energyloss = i*50.;
		p1.emitGamma(energyloss,0.);
                p1.setPosition(-500.,0,-CRANG,0,0);
                p1.computePath(beamline,1);

                TGraph * ppath_x = p1.getPath(0,kBlack);
                beam_profile_x->Add(rescale_tgraph(ppath_x,1E-3));
		char text[500];
		sprintf(text,"%d GeV",(int)energyloss);
		leg->AddEntry(ppath_x,text,"l");
	...
	ccc->cd();
        ...
        beam_profile_x->GetYaxis()->SetTitle("x [mm]");
        beam_profile_x->GetYaxis()->SetTitleOffset(0.75);
//	beam_profile_x->GetXaxis()->SetRangeUser(200,450);
//	beam_profile_x->GetYaxis()->SetRangeUser(-20,20);
//	leg->Draw();
	leg->SetFillColor(0);
}
